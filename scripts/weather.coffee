...
lookupWeather = (msg, coords, err) ->
  ...
  msg.http(url).query(units: 'ca').get() (err, res, body) ->
    ...
    feelsLike = getTemp(current.apparentTemperature)
    temperatureC = getTempC(current.temperature)
    feelsLikeC = getTempC(current.apparentTemperature)

    text = "It is currently #{temperature}/#{temperatureC} #{current.summary}, #{humidity}% humidity, #{current.windSpeed}mph #{current.windBearing}º, feels like: #{feelsLike}/#{feelsLikeC}"
    msg.send text
...
lookupForecast = (msg, coords, err) ->
  ...
  msg.http(url).query(units: 'ca').get() (err, res, body) ->
    ...
    appendText = (text, data) ->
      ...
      maxTempC = getTempC data.temperatureMax
      minTempC = getTempC data.temperatureMin

      text += "#{month}/#{day} - High of #{maxTemp}/#{maxTempC}, low of: #{minTemp}/#{minTempC} "
